...
class Commands {
		...
		static Option USER = newOption("u", "user", "Service user.", "user", true),
				...
				RETURN_CODE = newOption("rc", "return-code",
						format("If used with this option return code is %s "
								...
								+ "the transport is not modifiable. In this mode nothing is " + "emitted to STDOUT.",
								ExitException.ExitCodes.OK, ExitException.ExitCodes.FALSE),	null, false);

		static Option newOption(String shortKey, String longKey, String desc, String argName, boolean required) {
			return Option.builder(shortKey).hasArg(argName != null).argName(argName).longOpt(longKey).desc(desc).required(required).build();
		}
	}
	...
	static class Helpers {
		...
		static Options getStandardParameters(Options options, boolean optional) {
			Set<Option> standardOpts = newHashSet(CMOptions.USER, CMOptions.PASSWORD,
					CMOptions.HOST, CMOptions.HELP, CMOptions.VERSION);

			standardOpts.stream().forEach(o -> {
		...
		static void handleHelpOption(String commandName, String header, String args, Options options) {
			...
			String exitCodes = format("    %d  The request completed successfully.\n"
					...
					ExitException.ExitCodes.OK, ExitException.ExitCodes.GENERIC_FAILURE, ExitException.ExitCodes.NOT_AUTHENTIFICATED, ExitException.ExitCodes.FALSE);

			String commonOpts;

			HelpFormatter formatter = new HelpFormatter();

			try (StringWriter commonOptions = new StringWriter(); PrintWriter pw = new PrintWriter(commonOptions);) {
				formatter.printOptions(pw, formatter.getWidth(), Command.addOpts(new Options()),
			...
			formatter.printHelp(format("<CMD> [COMMON_OPTIONS] %s [SUBCOMMAND_OPTIONS] %s%n%n", commandName,
					args != null ? args : ""), format("SUBCOMMAND OPTIONS:%n", header), options, footer);
		 ...
		 * @return A copy of <code>args</code>. The password parameter, identified by a
		 *         preceding <code>-p</code> is replaced by asterisks.
		 */
	...
	static {
		...
		if (commands.stream().filter(it -> it.getAnnotation(CommandDescriptor.class) == null).findAny().isPresent())
			throw new IllegalStateException(
					format("Command without %s annotation found.", CommandDescriptor.class.getSimpleName()));
	}
	...
	public final static void main(String[] args) throws Exception {
		...
		if ((commandLine.hasOption(CMOptions.HELP.getOpt()) && args.length <= 1) || args.length == 0) {
			logger.debug("Printing help and return.");
			printHelp();
			if (args.length == 0)
				throw new CMCommandLineException("Called without arguments.");
			return;
		}
		...
		try {
			Optional<Class<? extends Command>> command = commands.stream().filter(it -> {
				CommandDescriptor a = it.getAnnotation(CommandDescriptor.class);
				return a.name().equals(commandName);
			}).findFirst();

			if (command.isPresent()) {
				logger.debug(format("Command name '%s' resolved to implementing class '%s'.", commandName,
						command.get().getName()));
				command.get().getDeclaredMethod("main", String[].class).invoke(null, new Object[] { args });
			} else {
				throw new CMCommandLineException(
						String.format("Command '%s' not found.", commandName));
			}
		} catch (InvocationTargetException e) {
			logger.error(format("Exception caught while executing command '%s': '%s'.", commandName, e.getMessage()),
					e);
			throw handle(e);
		} catch (Exception e) {
			logger.error(format("Exception caught while executing command '%s': '%s'.", commandName, e.getMessage()),
	...
	private static Exception handle(InvocationTargetException e) {
		if (e == null || e.getTargetException() == null)
		...
		return (e.getTargetException() instanceof Exception) ? (Exception) e.getTargetException()
	...
	private static String getCommandName(CommandLine commandLine, String[] args) throws ParseException {
		...
		if (commandLine.getArgs().length == 0) {
			throw new CMCommandLineException(
					format("Cannot extract command name from arguments: '%s'.", getArgsLogString(args)));
		}

		String commandName = commandLine.getArgs()[0];
		logger.debug(
				format("Command name '%s' extracted from command line '%s'.", commandName, getArgsLogString(args)));
		return commandName;
	}
	...
	private static void printHelp() {
		...
		commands.stream().map(it -> {
			CommandDescriptor cDesc = it.getAnnotation(CommandDescriptor.class);
			return format("%s", cDesc.name());
		}).sorted().forEach(subcmd -> subCommandsHelp.append(StringUtils.repeat(' ', 4)).append(subcmd).append(CRLF));
		...
		String footer = format("Subcommands:%s%s%sType '%s <subcommand> --help' for more details.%s", CRLF,
				subCommandsHelp.toString(), CRLF, cmd, CRLF);

		new HelpFormatter().printHelp(cmdLineSyntax, header, Helpers.getStandardOptions(), footer);
	}
}
